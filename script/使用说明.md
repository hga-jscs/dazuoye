...
```bash
python3 -m mytool <目标文件/目录> <下界> <上界> [忽略标签1 忽略标签2 ...]
```
...
```bash
python3 -m mytool "./json 230902" 3 5
```

**例子（单文件输入）：**

```bash
python3 -m mytool ./test/simple.json 2 4
```
...
```bash
python3 -m mytool ./json 230902 3 5 labelA labelB
```
...
- **运行模块**：配置运行目标为 `mytool`（等同于 `python3 -m mytool`）。
- **运行脚本**：也可直接运行 `script/mytool.py`（兼容旧入口）。
